...
torch.backends.cuda.enable_mem_efficient_sdp(mem_efficient_sdp)
...
data_dir = os.path.join(nfs_dir, 'data', dataset)
...
while True:
    ...
    if grad_clip != 0.0:
        ...
        grad_normf = torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip).item()
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        print(f"iter {iter_num}: loss {lossf:.4f}, grad_norm {grad_normf:.3f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
    iter_num += 1
    local_iter_num += 1
